...
class AnalyticsEngine:
    ...
    def run_simulation(self, h_stats, a_stats, lg_stats, params, h_id, a_id, league_code, roster_factor, high_precision=False):
        ...
        if high_precision:
            sims = 50000; sim_h = np.random.poisson(xg_h, sims); sim_a = np.random.poisson(xg_a, sims)
            p_home = np.mean(sim_h > sim_a) * 100; p_draw = np.mean(sim_h == sim_a) * 100; p_away = np.mean(sim_h < sim_a) * 100
            o25 = np.mean((sim_h + sim_a) > 2.5) * 100; btts = np.mean((sim_h > 0) & (sim_a > 0)) * 100
        ...
        return {"probs": {"1": p_home, "X": p_draw, "2": p_away, "o25": o25, "btts": btts}, "xg": (xg_h, xg_a), "elo": (elo_h, elo_a), "score": f"{np.unravel_index(np.argmax(matrix), matrix.shape)[0]}-{np.unravel_index(np.argmax(matrix), matrix.shape)[1]}", "matrix": matrix, "vectors": (has, hdw, aas, adw), "narrative": narrative, "entropy": entropy, "ci": (ci_h, ci_a)}
...
def save_pred_db(m, probs, params, user, meta):
    ...
    if not doc.get().exists:
        doc.set({"match_id": str(m['id']), "match_name": f"{meta['hn']} vs {meta['an']}", "match_date": m['utcDate'], "league": meta['lg'], "home_prob": p1, "draw_prob": p2, "away_prob": p3, "predicted_outcome": ("1" if p1>p2 and p1>p3 else "2" if p3>p1 and p3>p2 else "X"), "confidence": meta['conf'], "dqi": meta['dqi'], "user": user, "params": str(params), "model_version": MODEL_VERSION, "actual_result": None}, merge=True)

# --- 4. VISUALS ---
def create_score_heatmap(matrix, h_name, a_name):
    return go.Figure(data=go.Heatmap(z=matrix[:6, :6], x=[str(i) for i in range(6)], y=[str(i) for i in range(6)], colorscale='Viridis', showscale=False, texttemplate="%{z:.1%}")).update_layout(title=f"{h_name} vs {a_name} Olasılık", xaxis_title=f"{a_name}", yaxis_title=f"{h_name}", width=400, height=400, font=dict(color="white"), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')

def create_radar(hs, as_, vectors):
    has, hdw, aas, adw = vectors; categories = ['Hücum', 'Savunma', 'Form', 'Elo']
...
def main():
    ...
    with st.sidebar:
        ...
        st.divider(); high_prec = st.checkbox("Yüksek Hassasiyet (Monte Carlo)", help="50k Simülasyon")

    st.title("QUANTUM FOOTBALL"); st.caption(f"v{MODEL_VERSION} | AI-Powered Lab")
    ...
    if nav == t["nav_sim"]:
        ...
        if fixtures:
            ...
            if not upc: st.warning("Maç yok.")
                ...
                with st.expander("🛠️ Parametreler"):
                    ...
                    roster = ct3.selectbox("Kadro", ["Tam", "Ev Eksik", "Dep Eksik"])
                    roster_f = (0.85, 1.0) if roster == "Ev Eksik" else (1.0, 0.85) if roster == "Dep Eksik" else (1.0, 1.0)
                ...
                if st.button(t["btn_sim"]):
                    ...
                    st.subheader(t["scenarios"]); m_df = pd.DataFrame({"Senaryo": ["2.5 Üst", "KG Var"], "Olasılık (%)": [f"%{res['probs']['o25']:.1f}", f"%{res['probs']['btts']:.1f}"]}); st.table(m_df)
        ...
        if db:
            ...
            if docs:
                ...
                c1, c2, c3 = st.columns(3); c1.metric("Maç", total); c2.metric("İsabet", f"%{(correct/total)*100:.1f}"); c3.metric("RPS", f"{rps_sum/total:.3f}")
                st.subheader("Kalibrasyon"); df_cal = pd.DataFrame(cal_data); df_cal['bin'] = pd.cut(df_cal['prob'], bins=np.arange(0, 101, 10))
                cal_plot = df_cal.groupby('bin').agg({'correct': 'mean', 'prob': 'mean'}).reset_index()
                st.plotly_chart(px.scatter(cal_plot, x='prob', y='correct', title="Güvenilirlik", labels={'prob': 'Güven', 'correct': 'Gerçekleşme'}).add_shape(type="line", x0=0, y0=0, x1=100, y1=1, line=dict(color="Red", dash="dash")), use_container_width=True)
                st.dataframe(pd.DataFrame([{"Maç": d.to_dict().get("match_name"), "Tahmin": d.to_dict().get("predicted_outcome"), "Sonuç": d.to_dict().get("actual_result")} for d in docs]))
