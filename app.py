...
CONFIG = {
    ...
    "ADMIN_PASS": "muratLola26",  # İstenilen Şifre
    "LOG_FILE": "user_activity.csv" # Log Dosyası
}

st.set_page_config(page_title="Quantum Football", page_icon="⚽", layout="wide")

# -----------------------------------------------------------------------------
# 2. KULLANICI TAKİP VE LOGLAMA SİSTEMİ (YENİ EKLENDİ)
# -----------------------------------------------------------------------------
# URL'den (HTML tarafından gönderilen) kullanıcı mailini yakala
query_params = st.query_params
current_user = query_params.get("user_email", "Misafir_Kullanici")

def log_activity(league, match, h_att, a_att):
    """Analiz butonuna basıldığında veriyi kaydeder"""
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    # Bazen query param liste olarak gelebilir, string'e çevir
    user_str = current_user[0] if isinstance(current_user, list) else current_user
    
    new_data = [timestamp, user_str, league, match, f"Ev:{h_att}", f"Dep:{a_att}"]
    ...
    try:
        with open(CONFIG["LOG_FILE"], mode='a', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            if not file_exists:
                writer.writerow(["Zaman", "Kullanici", "Lig", "Mac", "Form H", "Form A"])
...
TRANSLATIONS = {
    "tr": {
        ...
        "footer": f"Quantum Football v100.0 | User: {current_user}"
    },
    "en": {
        "app_title": "QUANTUM FOOTBALL",
        "settings": "Settings",
...
def main():
        ...
        with st.expander("🔐 Admin Paneli"):
            ...
            if admin_pw == CONFIG["ADMIN_PASS"]:
                ...
                if os.path.exists(CONFIG["LOG_FILE"]):
                    st.write("### 📊 Kullanıcı Aktiviteleri")
                    ...
                    st.info("Henüz kayıtlı bir analiz yok.")
            elif admin_pw:
    ...
    # KULLANICI ADI GÖSTERİMİ (Varsa)
    if current_user and current_user != "Misafir_Kullanici":
        user_display = current_user[0] if isinstance(current_user, list) else current_user
        st.caption(f"👤 Aktif Kullanıcı: **{user_display}**")

    dm = DataManager(api_key)
    
    # --- LİG LİSTESİ (SADECE STANDART OLANLAR) ---
    L_MAP = {
    ...
    if st.button(f"{t['start_btn']} ({sim_count//1000}K)", use_container_width=True):
        ...
        # --- LOGLAMA İŞLEMİ ---
        log_activity(league, sel_match, h_att, a_att)
